...
def get_sys_argv(pp_args=[]):
    ...
    if len(p_args) > 5:
        pregion = p_args[5]
        if pregion not in list_regions and pregion != 'ALL':
...
def main(testing=False):
    ...
    txt_header = "{0:<15} {1:<6} {2:<6} {3:<10} {4:<8} {5:<11} {6:<8} {7:<10} {8} {9} {10} {11}" \
                              .format("Instance", "vCPU", "RAM", "OS", "PriceH", "PriceM", "SpotH", "SpotM", "KillRate","Region","Timestamp" ,"Spot%Discount" )
        ...
        try:
            ...
            if text_only:
                ...
                for rr in result:
                    ...
                    spotprice_hourly = float(spot_prices[rr[1]]['SpotPrice'])
                    timestamp = spot_prices[rr[1]]['Timestamp']
                    spotprice_monthly = spotprice_hourly * 24 * 30
                    spotdiscount = 100*(rr[5]-spotprice_hourly)/rr[5]
                    kill_rate = spot_interrupt_rates[rr[1]]
                    print(Fore.GREEN + "{0: <15} {1:<6.2f} {2:<6.2f} {3: <10} {4:.5f}  {5:<10.5f}  {6:.5f}  {7:<10.5f} {8:<3} {9: <10} {10} {11} "
                                        .format(rr[1], rr[2], rr[3], rr[4], rr[5], rr[5] * 24 * 30,
                                        spotprice_hourly, spotprice_monthly, kill_rate,region_map[pregion],timestamp,spotdiscount ))
